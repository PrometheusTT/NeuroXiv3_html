...
function getGeneObj (loadingTarget: HTMLElement | null, condition:any, requestOptions: RequestOptions = {}) {
  const url = `${REQUEST_NAME_SPACE}generate_obj`
  const options: RequestInit = {
    method: 'post',
    body: JSON.stringify(condition)
  }
  requestOptions.errorMsg = requestOptions.errorMsg || 'get GeneObj error'
  const params = { url, loadingTarget, options, ...requestOptions }
  let r = request(params)
  let originR = r.start.bind(r)
  r.start = () => originR().then((data: any) => {
